...
if __name__ == '__main__':
    ...
    for (lmbda, delta), plot_res in plots:
        curves.plot_loss(plot_res, ty='train', log=True, step=4, last=None,
                         small=True, legend=delta > 0.2, title='imdb, $\delta$ = {:.2f}'.format(delta))
        plt.savefig('/home/thoth/abietti/shared/imdb_dropout_{:.2f}.pdf'.format(delta), format='pdf', bbox_inches='tight', pad_inches=0)
        pp.savefig()
