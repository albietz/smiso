...
#define checkCusolver(status) do {                                        \
   std::stringstream _error;                                          \
   if (status != 0) {                              \
      _error << "Cusolver failure: " << status << endl;      \
...
template <typename T> class AbstractMatrixB {
   public:
      ...
      virtual void print_dims(const string& name);
...
template<typename T> class Vector {
   ...
   //inline void print(const char* name) const;
   inline void print(const string& name) const;
   inline void print_dim(const string& name) const;
...
/// Print the vector dimensions to std::cout
template <typename T> inline void Vector<T>::print_dim(const string& name) const {
   std::cerr << name << ": " << _n << std::endl;
};
...
/// print the matrix dimensions to cout
template <typename T> void AbstractMatrixB<T>::print_dims(const string& name) {
  std::cout << name << ": " << m() << " x " << m() << std::endl;
}
...
template<typename T> class CudaVector {
   ...
   public:
      ...
      inline void setData(T* X, INTM n) { 
         this->clear(); 
         _X=X;
         _n=n;
         _externAlloc=true;
      };
      inline void setVector(const Vector<T>& vec);
      inline void getVector(Vector<T>& vec) const;
      inline void resize(INTM n);
      inline void clear();
      inline void setZeros();
      inline void set(const T a);
      inline void customset(const T a);
      inline void custom(const T a);
      inline void custom2(const T a);
      inline void inv();
      inline void inv_sqrt();
      inline void inv_sqrt_add(const T a);
      inline void exp();
      inline T nrm2sq();
      inline void copy(const CudaVector<T>& mat);
      inline T* rawX() const { return _X; };
      inline void print(const string& name) const;
      inline T dot(const CudaVector<T>& x);
      inline void scal(const T a);
      inline void add(const CudaVector<T>& x, const T a);
...
template<typename T> inline void CudaVector<T>::getVector(Vector<T>& vec) const {
   vec.resize(_n);
   cublas_stat = cublasGetVector (_n, sizeof(T), _X,1, vec.rawX(), 1);
};

template<typename T> inline void CudaVector<T>::setVector(const Vector<T>& vec) {
   resize(vec.n());
   cublas_stat = cublasSetVector (_n, sizeof(T), vec.rawX(), 1, _X, 1);
};

/// Change the data in the matrix
template <typename T> inline void CudaVector<T>::resize(const INTM n) {
   if (_n == n) return;
   clear();
   _n=n;
   _externAlloc=false;
   cudaStat = cudaMalloc ((void**)&_X, _n*sizeof(T));
...
/// Change the data in the matrix
template <typename T> inline void CudaVector<T>::clear() {
   if (!_externAlloc)
      cudaFree(_X);
   _n=0;
   _externAlloc=true;
...
template <typename T> inline void CudaVector<T>::setZeros() {
   cudaMemset(_X,0,_n*sizeof(T));
};

template <typename T> inline void CudaVector<T>::set(const T a) {
   cuda_set(_n,_X,a);
};

template <typename T> inline void CudaVector<T>::customset(const T a) {
   setZeros();
   cuda_set(_n-1,_X,a);
};

template <typename T> inline void CudaVector<T>::custom(const T thrs) {
   cuda_custom(_n,_X,thrs);
};

template <typename T> inline void CudaVector<T>::custom2(const T thrs) {
   cuda_custom2(_n,_X,thrs);
};


template <typename T> inline void CudaVector<T>::print(const string& name) const {
   Vector<T> vec;
   getVector(vec);
   vec.print(name);
};

template <> inline double CudaVector<double>::nrm2sq() {
   double result;
   cublasDdot(handle,_n,_X,1,_X,1,&result);
   return result;
};

template <> inline float CudaVector<float>::nrm2sq() {
   float result;
   cublasSdot(handle,_n,_X,1,_X,1,&result);
   return result;
};

template <typename T> inline void CudaVector<T>::inv() {
   cuda_inv(_n,_X);
};
template <typename T> inline void CudaVector<T>::inv_sqrt() {
   cuda_inv_sqrt(_n,_X);
};

template <typename T> inline void CudaVector<T>::inv_sqrt_add(const T a) {
   cuda_inv_sqrt_add(_n,_X,a);
};

template <typename T> inline void CudaVector<T>::copy(const CudaVector<T>& vec) {
   resize(vec._n);
   cudaMemcpy(_X,vec._X,_n*sizeof(T),cudaMemcpyDeviceToDevice);
};

template <> inline double CudaVector<double>::dot(const CudaVector<double>& x) {
   double result;
   cublasDdot(handle,_n,_X,1,x._X,1,&result);
   return result;
};

template <> inline float CudaVector<float>::dot(const CudaVector<float>& x) {
   float result;
   cublasSdot(handle,_n,_X,1,x._X,1,&result);
   return result;
};

template <> inline void CudaVector<double>::scal(const double a) {
   cublasDscal(handle,_n,&a,_X,1);
};

template <> inline void CudaVector<float>::scal(const float a) {
   cublasSscal(handle,_n,&a,_X,1);
};

template <> inline void CudaVector<double>::add(const CudaVector<double>& x, const double a) {
   cublasDaxpy(handle,_n,&a,x._X,1,_X,1);
};

template <> inline void CudaVector<float>::add(const CudaVector<float>& x, const float a) {
   cublasSaxpy(handle,_n,&a,x._X,1,_X,1);
};


template<typename T> inline void CudaVector<T>::exp() {
   cuda_exp(_n,_X);
};
...
template<typename T> class CudaMatrix {
   ...
   public:
      /// Constructor with existing data X of an m x n matrix
      CudaMatrix();
      CudaMatrix(T* X, int m, int n);
      
      /// Destructor
      virtual ~CudaMatrix();

      inline void setData(T* X, INTM m , INTM n);
      inline void refMat(CudaMatrix<T>& mat);
      inline void setMatrix(const Matrix<T>& mat);
      inline void getMatrix(Matrix<T>& mat) const;
      inline void resize(INTM m, INTM n);
      inline void copy(const CudaMatrix<T>& mat);
      inline void clear();
      inline void exp();
      inline void add_exp(const T a);
      inline void sqr();
      inline void sqr(CudaMatrix<T>& out);
      inline void setZeros();
      inline void set(const T a);
      inline void scal(const T a);
      inline T asum() const;
      inline void eigSym(CudaMatrix<T>& U, CudaVector<T>& S) const;
      inline void multDiagRight(const CudaVector<T>& R);
      inline void multDiagRight(const CudaVector<T>& R, CudaMatrix<T>& out);
      inline void multDiagLeft(const CudaVector<T>& R);
      inline void rank1Update(const CudaVector<T>& x, const CudaVector<T>& y,const T alpha =T(1.0));
      inline void thrsmax(const T nu);
      inline void add(const CudaMatrix<T>& A, const T a = 1.0);
      inline void print(const string& name) const;
      inline T normF() const;
      inline void custom_relu(CudaMatrix<T>& A, CudaMatrix<T>& B);
      
      inline void mult_elementWise(const CudaMatrix<T>& B);
      inline void mult(const CudaMatrix<T>& B, CudaMatrix<T>& C, 
            const bool transA = false, const bool transB = false,
            const T a = 1.0, const T b = 0.0) const;
      inline void multTrans(const CudaVector<T>& x, CudaVector<T>& b, 
            const T a = 1.0, const T c = 0.0) const;
      inline void mult(const CudaVector<T>& x, CudaVector<T>& b, 
            const T a = 1.0, const T c = 0.0) const;
//      inline void solveSymPos(const CudaMatrix<T>& eye, CudaMatrix<T>& invA);
      inline void solveSymPos(const CudaMatrix<T>& X, CudaMatrix<T>& Y);
      inline void diag(CudaVector<T>& diag) const;
      inline void addDiag(const CudaVector<T>& diag, const T alpha = 1.0);
      /// out = X + X';
      inline void addTranspose(CudaMatrix<T>& out);
      //inline void normalize2spec(const T thrs = 1.0);
      inline void copy_allocation();
      ...
      inline INTM n() const { return _n; };
      inline T* rawX() const { return _X; };
      inline void reshape(const int m, const int n) {
         if (m*n != _m*_n) cout << "Problem with sizes" << endl;
         _m=m;
         _n=n;
      }
   private:
...
template<typename T> CudaMatrix<T>::CudaMatrix(T* X, int m, int n) {
   _externAlloc=true;
   _m=m;
   _n=n;
   _X=X;
...
template<typename T> inline void CudaMatrix<T>::setData(T* X, INTM m, INTM n) {
   this->clear();
   _externAlloc=true;
   _m=m;
   _n=n;
...
template<typename T> inline void CudaMatrix<T>::copy_allocation() {
   if (_externAlloc) {
      CudaMatrix<T> copymatrix(_X,_m,_n);
      this->clear();
...
template <typename T> inline void CudaMatrix<T>::setZeros() {
   cudaMemset(_X,0,_m*_n*sizeof(T));
};
...
template<typename T> inline void CudaMatrix<T>::getMatrix(Matrix<T>& mat) const {
   mat.resize(_m,_n);
   cublas_stat = cublasGetMatrix (_m, _n, sizeof(T), _X, _m, mat.rawX(), _m);
};

template<typename T> inline void CudaMatrix<T>::setMatrix(const Matrix<T>& mat) {
   resize(mat.m(),mat.n());
   cublas_stat = cublasSetMatrix (_m, _n, sizeof(T), mat.rawX(), _m, _X, _m);
};

template<typename T> inline void CudaMatrix<T>::thrsmax(const T nu) {
   cuda_thrsmax(_m*_n,_X,nu);
};

template<typename T> inline void CudaMatrix<T>::exp() {
   cuda_exp(_m*_n,_X);
};

template<typename T> inline void CudaMatrix<T>::add_exp(const T a) {
   cuda_add_exp(_m*_n,_X,a);
};


template <typename T> inline void CudaMatrix<T>::set(const T a) {
   cuda_set(_m*_n,_X,a);
};

template<typename T> inline void CudaMatrix<T>::sqr() {
   cuda_sqr(_m*_n,_X);
};

template<typename T> inline void CudaMatrix<T>::sqr(CudaMatrix<T>& out) {
   out.resize(_m,_n);
   cuda_sqr(_m*_n,_X,out._X);
};



template <typename T> inline void CudaMatrix<T>::mult_elementWise(const CudaMatrix<T>& B) {
   cuda_mult(_m*_n,_X,B._X);
}

template <typename T> inline void CudaMatrix<T>::custom_relu(CudaMatrix<T>& A, CudaMatrix<T> & B) {
   resize(A.m(),A.n());
   cuda_custom_relu(_m*_n,_X,A._X,B._X);
}

template <> inline double CudaMatrix<double>::asum() const {
   double* sum;
   cublasDasum(handle,_m*_n,_X,1,sum);
   return *sum;
};

template <> inline float CudaMatrix<float>::asum() const {
   float* sum;
   cublasSasum(handle,_m*_n,_X,1,sum);
   return *sum;
};


template <> inline void CudaMatrix<double>::multTrans(const CudaVector<double>& x, CudaVector<double>& b, 
      const double a, const double c) const {
   b.resize(_n);
   cublasDgemv(handle,CUBLAS_OP_T,_m,_n,&a,_X,_m,x._X,1,&c,b._X,1);
};

template <> inline void CudaMatrix<float>::multTrans(const CudaVector<float>& x, CudaVector<float>& b, 
      const float a, const float c) const {
   b.resize(_n);
   cublasSgemv(handle,CUBLAS_OP_T,_m,_n,&a,_X,_m,x._X,1,&c,b._X,1);
};

template <> inline void CudaMatrix<double>::mult(const CudaVector<double>& x, CudaVector<double>& b, 
      const double a, const double c) const {
   b.resize(_m);
   cublasDgemv(handle,CUBLAS_OP_N,_m,_n,&a,_X,_m,x._X,1,&c,b._X,1);
};

template <> inline void CudaMatrix<float>::mult(const CudaVector<float>& x, CudaVector<float>& b, 
      const float a, const float c) const {
   b.resize(_m);
   cublasSgemv(handle,CUBLAS_OP_N,_m,_n,&a,_X,_m,x._X,1,&c,b._X,1);
};

template <> inline double CudaMatrix<double>::normF() const {
   double result;
   cublasDnrm2(handle,_m*_n,_X,1,&result);
   return result;
};
template <> inline float CudaMatrix<float>::normF() const {
   float result;
   cublasSnrm2(handle,_m*_n,_X,1,&result);
   return result;
};
template <> inline void CudaMatrix<double>::scal(const double a) {
   cublasDscal(handle,_m*_n,&a,_X,1);
};
template <> inline void CudaMatrix<float>::scal(const float a) {
   cublasSscal(handle,_m*_n,&a,_X,1);
};
template <> inline void CudaMatrix<double>::mult(const CudaMatrix<double>& B, CudaMatrix<double>& C, 
      const bool transA, const bool transB,
      const double a, const double b) const {
   int m,k,n;
   cublasOperation_t trA, trB;
   if (transA) {
      trA = CUBLAS_OP_T;
      m = _n;
      k = _m;
   ...
   C.resize(m,n);
   cublas_stat=cublasDgemm(handle,trA, trB,m,n,k, &a,_X,_m,B._X,B._m,&b,C._X,C._m);
};

template <> inline void CudaMatrix<float>::mult(const CudaMatrix<float>& B, CudaMatrix<float>& C, 
      const bool transA, const bool transB,
   ...
   //PRINT_I(n)
   C.resize(m,n);
   cublas_stat=cublasSgemm(handle,trA, trB,m,n,k, &a,_X,_m,B._X,B._m,&b,C._X,C._m);
};

/// Change the data in the matrix
template <typename T> inline void CudaMatrix<T>::refMat(CudaMatrix<T>& mat) {
   clear();
   _X=mat._X;
   _m=mat._m;
   _n=mat._n;
...
template <typename T> inline void CudaMatrix<T>::copy(const CudaMatrix<T>& mat) {
   resize(mat._m,mat._n);
   cudaMemcpy(_X,mat._X,_m*_n*sizeof(T),cudaMemcpyDeviceToDevice);
}

/// Change the data in the matrix
template <typename T> inline void CudaMatrix<T>::resize(const INTM m, const INTM n) {
   if (_m == m && _n == n) return;
   clear();
   _m=m;
   _n=n;
   _externAlloc=false;
   cudaStat = cudaMalloc ((void**)&_X, _m*_n*sizeof(T));
   if (cudaStat != cudaSuccess) { 
      printf ("device memory allocation failed"); 
      exit(1);
   }
...
/// Change the data in the matrix
template <typename T> inline void CudaMatrix<T>::clear() {
   if (!_externAlloc)
      cudaFree(_X);
   _m=0;
   _n=0;
...
template <> inline void CudaMatrix<double>::multDiagRight(const CudaVector<double>& R) {
   cublas_stat=cublasDdgmm(handle,CUBLAS_SIDE_RIGHT,_m,_n,_X,_m,R.rawX(),1,_X,_m);
};

template <> inline void CudaMatrix<float>::multDiagRight(const CudaVector<float>& R) {
   cublas_stat=cublasSdgmm(handle,CUBLAS_SIDE_RIGHT,_m,_n,_X,_m,R.rawX(),1,_X,_m);
};

template <> inline void CudaMatrix<double>::multDiagRight(const CudaVector<double>& R, CudaMatrix<double>& out) {
   out.resize(_m,_n);
   cublas_stat=cublasDdgmm(handle,CUBLAS_SIDE_RIGHT,_m,_n,_X,_m,R.rawX(),1,out._X,_m);
};

template <> inline void CudaMatrix<float>::multDiagRight(const CudaVector<float>& R, CudaMatrix<float>& out) {
   out.resize(_m,_n);
   cublas_stat=cublasSdgmm(handle,CUBLAS_SIDE_RIGHT,_m,_n,_X,_m,R.rawX(),1,out._X,_m);
};


template <> inline void CudaMatrix<double>::multDiagLeft(const CudaVector<double>& R) {
   cublas_stat=cublasDdgmm(handle,CUBLAS_SIDE_LEFT,_m,_n,_X,_m,R.rawX(),1,_X,_m);
};

template <> inline void CudaMatrix<float>::multDiagLeft(const CudaVector<float>& R) {
   cublas_stat=cublasSdgmm(handle,CUBLAS_SIDE_LEFT,_m,_n,_X,_m,R.rawX(),1,_X,_m);
};
template <> inline void CudaMatrix<double>::rank1Update(const CudaVector<double>& x, 
      const CudaVector<double>& y,const double alpha) {
   cublas_stat=cublasDger(handle,_m,_n,&alpha,x.rawX(),1,y.rawX(),1,_X,_m);
};

template <> inline void CudaMatrix<float>::rank1Update(const CudaVector<float>& x, 
      const CudaVector<float>& y,const float alpha) {
   cublas_stat=cublasSger(handle,_m,_n,&alpha,x.rawX(),1,y.rawX(),1,_X,_m);
};


template <> inline void CudaMatrix<double>::add(const CudaMatrix<double>& A, const double a){
   cublas_stat=cublasDaxpy(handle,_n*_m,&a,A._X,1,_X,1);
};

template <> inline void CudaMatrix<float>::add(const CudaMatrix<float>& A, const float a){
   cublas_stat=cublasSaxpy(handle,_n*_m,&a,A._X,1,_X,1);
};

template <typename T> inline void CudaMatrix<T>::print(const string& name) const {
   Matrix<T> vec;
   getMatrix(vec);
   vec.print(name);
};
...
template <> 
inline void CudaMatrix<double>::solveSymPos(const CudaMatrix<double>& X, CudaMatrix<double>& Y) {
   int lwork = 0;
   cusolver_status = cusolverDnDpotrf_bufferSize(cusolver_handle, CUBLAS_FILL_MODE_UPPER, _m, _X, _m, &lwork );
   double* work;
   cudaStat = cudaMalloc ((void**)&work, lwork*sizeof(double));
   int* pr_devInfo;
   cudaStat = cudaMalloc ((void**)&pr_devInfo, sizeof(int));
   cusolver_status = cusolverDnDpotrf(cusolver_handle, CUBLAS_FILL_MODE_UPPER, _m, _X, _m, work, lwork, pr_devInfo); 
   Y.copy(X);
   cusolver_status = cusolverDnDpotrs(cusolver_handle,  CUBLAS_FILL_MODE_UPPER, _m, Y.n(), _X, _m, Y.rawX(), _m, pr_devInfo);
   cudaFree(work);
   cudaFree(pr_devInfo);
};

template <> 
inline void CudaMatrix<float>::solveSymPos(const CudaMatrix<float>& X, CudaMatrix<float>& Y) {
   int lwork = 0;
   cusolver_status = cusolverDnSpotrf_bufferSize(cusolver_handle, CUBLAS_FILL_MODE_UPPER, _m, _X, _m, &lwork );
   float* work;
   cudaStat = cudaMalloc ((void**)&work, lwork*sizeof(float));
   int* pr_devInfo;
   cudaStat = cudaMalloc ((void**)&pr_devInfo, sizeof(int));
   cusolver_status = cusolverDnSpotrf(cusolver_handle, CUBLAS_FILL_MODE_UPPER, _m, _X, _m, work, lwork, pr_devInfo); 
   Y.copy(X);
   cusolver_status = cusolverDnSpotrs(cusolver_handle,  CUBLAS_FILL_MODE_UPPER, _m, Y.n(), _X, _m, Y.rawX(), _m, pr_devInfo);
   cudaFree(work);
   cudaFree(pr_devInfo);
};

template <> 
inline void CudaMatrix<float>::eigSym(CudaMatrix<float>& U, CudaVector<float>& S) const {
   U.resize(_m,_m);
   S.resize(_m);
   int lwork = 0;
#ifdef CUDA_8
   checkCusolver(cusolverDnSsyevd_bufferSize(cusolver_handle, CUSOLVER_EIG_MODE_VECTOR, CUBLAS_FILL_MODE_UPPER, _m, _X, _m, S._X, &lwork));
   float* work;
   cudaStat = cudaMalloc ((void**)&work, lwork*sizeof(float));
   int* pr_devInfo;
   cudaStat = cudaMalloc ((void**)&pr_devInfo, sizeof(int));
   U.copy(*this);
   checkCusolver(cusolverDnSsyevd(cusolver_handle,CUSOLVER_EIG_MODE_VECTOR, CUBLAS_FILL_MODE_UPPER, _m, U._X, _m, S._X, work, lwork, pr_devInfo));
   cudaFree(work);
   cudaFree(pr_devInfo);
#else
   cout << "Eigenvalue solver is not implemented " << endl;
#endif
};

template <> 
inline void CudaMatrix<double>::eigSym(CudaMatrix<double>& U, CudaVector<double>& S) const {
   U.resize(_m,_m);
   S.resize(_m);
   int lwork = 0;
#ifdef CUDA_8
   checkCusolver(cusolverDnDsyevd_bufferSize(cusolver_handle, CUSOLVER_EIG_MODE_VECTOR, CUBLAS_FILL_MODE_UPPER, _m, _X, _m, S._X, &lwork));
   double* work;
   cudaStat = cudaMalloc ((void**)&work, lwork*sizeof(double));
   int* pr_devInfo;
   cudaStat = cudaMalloc ((void**)&pr_devInfo, sizeof(int));
   U.copy(*this);
   checkCusolver(cusolverDnDsyevd(cusolver_handle,CUSOLVER_EIG_MODE_VECTOR, CUBLAS_FILL_MODE_UPPER, _m, U._X, _m, S._X, work, lwork, pr_devInfo));
   cudaFree(work);
   cudaFree(pr_devInfo);
#else
   cout << "Eigenvalue solver is not implemented " << endl;
...
template <>
inline void CudaMatrix<double>::diag(CudaVector<double>& diag) const {
   diag.resize(_m);
   cublasDcopy(handle,_m,_X,_m+1,diag.rawX(),1);
};
template <>
inline void CudaMatrix<float>::diag(CudaVector<float>& diag) const {
   diag.resize(_m);
   cublasScopy(handle,_m,_X,_m+1,diag.rawX(),1);
};

template <>
inline void CudaMatrix<double>::addDiag(const CudaVector<double>& diag, const double alpha) {
   cublasDaxpy(handle,_m,&alpha,diag.rawX(),1,_X,_m+1);
};
template <>
inline void CudaMatrix<float>::addDiag(const CudaVector<float>& diag, const float alpha) {
   cublasSaxpy(handle,_m,&alpha,diag.rawX(),1,_X,_m+1);
};
template <>
inline void CudaMatrix<double>::addTranspose(CudaMatrix<double>& out) {
   out.resize(_m,_m);
   double alpha=1.0;
   cublasDgeam(handle,CUBLAS_OP_N,CUBLAS_OP_T,_m,_m,&alpha,_X,_m,&alpha,_X,_m,out._X,_m);
};
template <>
inline void CudaMatrix<float>::addTranspose(CudaMatrix<float>& out) {
   out.resize(_m,_m);
   float alpha=1.0;
   cublasSgeam(handle,CUBLAS_OP_N,CUBLAS_OP_T,_m,_m,&alpha,_X,_m,&alpha,_X,_m,out._X,_m);
};
