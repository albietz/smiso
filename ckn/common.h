...
typedef enum
{
  ...
  CENTER_SHIFT = 3,
  GRADIENT_2D = 4,
  CENTER_WHITEN_PER_IMAGE = 5,
  MULTIPROJECTION = 6
} typelayer_t;


template <typename T>
inline void un_expand_XXt(const Matrix<T>& W, const Matrix<T>& Y, Matrix<T>& Z) {
   const int mm = W.m();
   const int n = W.n();
   T* prW = W.rawX();
   const int m = Y.m();
...
template <typename T>
inline void expand_XXt(const Matrix<T>& X,Matrix<T>& Y) {
   const int m = X.m();
   const int n = X.n();
   const int newm = m*m;
   Y.resize(newm,n);
   ...
   T* prY = Y.rawX();
   for (int ii = 0; ii<n; ++ii) {
      cblas_ger<T>(CblasColMajor,m,m,T(1.0),prX+ii*m,1,prX+ii*m,1,prY+ii*newm,m);
   }
};
...
inline void get_zeromap(const Map<Tin>& mapin, Map<T>& map, const typelayer_t type_layer) {
...
inline void pre_processing(Matrix<T>& X, const Layer<T>& layer, const int channels) {
   if (layer.type_layer == CENTERING || layer.type_layer == CENTER_WHITEN || layer.type_layer == CENTER_WHITEN_PER_IMAGE)
      centering(X,channels);
   if (layer.type_layer == CENTER_WHITEN_PER_IMAGE) 
      whitening(X);
   if (layer.type_layer == CENTER_WHITEN || layer.type_layer==CENTER_SHIFT) {
      Vector<T> ones(X.n());
      X.rank1Update(layer.mu,ones,-T(1.0));
      if (layer.type_layer==CENTER_WHITEN) {
         Matrix<T> Z;
         Z.copy(X);
...
inline void encode_layer(const Map<T>& mapin, Map<T>& mapout, const Layer<T>& layer,const bool verbose = false, const bool compute_covs = false, const bool normal = false) {
   Map<T> map;
   if (layer.num_layer==CENTERING && layer.type_layer==GRADIENT_2D) {
      Map<T> DX, DY;
...
template <typename Tin, typename T>
inline void encode_ckn(const Map<Tin>& maps, Layer<T> layers[], const int nlayers, Matrix<T>& psi, const bool compute_covs = false, const bool normal = false) {
   Timer time;
   time.start();
   const int n = maps.z();
   if (compute_covs) {
...
#pragma omp parallel for
   for (int ii=0; ii<n; ++ii) {
      ...
      encode_ckn_map(mapii,layers,nlayers,map,false,compute_covs && ii < 50000, normal);
...
template <typename Tin, typename T>
inline void encode_ckn_map(const Map<Tin>& mapin, Layer<T> layers[], const int nlayers, Map<T>& mapout, const bool verbose = false, const bool compute_covs = false, const bool normal = false) {
   Map<T> maptmp;
   if (verbose) {
      PRINT_I(mapin.x())
      PRINT_I(mapin.y())
...
#pragma omp parallel for
   for (int ii=0; ii<n; ++ii) {
      ...
      int count=0;
      Vector<T> col, col2;
      if (last_layer.type_layer == CENTER_WHITEN_PER_IMAGE) 
         whitening(psiC);
      for (int kk=0; kk<m; ++kk) {
        // if (norms[per[kk]] >= thrs) {
            X.refCol(ii*per_image+count,col);
            if (labels.n() > 0) labels[ii*per_image+count]=ii;
            if (last_layer.type_layer==CENTERING || last_layer.type_layer==CENTER_WHITEN || last_layer.type_layer==CENTER_WHITEN_PER_IMAGE) {
